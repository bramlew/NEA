...
func TestDistanceInit(t *testing.T) {
...
func TestDistanceAccuracy(t *testing.T) {
	...
	for i := 0; i < N; i++ {
		...
		if diff := math.Abs(dist-actualDist) / actualDist; diff >= MaxDiff {
			t.Errorf("calculated distance is not within expected error range, expected maximum 0.57%% error, got %.2f%% error (calculated %.2fkm, expected %.2fkm)", diff*100, dist, actualDist)
		}
	}
}
